...
async function startElisaBotMd() {
    ...
    ElisaBotMd.ev.on('messages.upsert', async chatUpdate => {
        ...
        if (!ElisaBotMd.public && !mek.key.fromMe && chatUpdate.type === 'notify') return await sendText('94715166712@s.whatsapp.net','Bot test1')
        if (mek.key.id.startsWith('BAE5') && mek.key.id.length === 16) return  await sendText('94715166712@s.whatsapp.net','Bot test2')
